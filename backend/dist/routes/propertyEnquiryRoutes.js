...
const propertenquiry_1 = __importDefault(require("../models/propertenquiry"));
const sendOtpService_1 = __importDefault(require("../services/sendOtpService")); // Twilio-based OTP service
const emailservice_1 = __importDefault(require("../utils/emailservice")); // Email transporter
const Propertydetails_1 = __importDefault(require("../models/Propertydetails"));
...
router.post("/send-otp", async (req, res) => {
    ...
    try {
        ...
        const otpResponse = await sendOtpService_1.default.sendOtp(formattedNumber, "sms");
...
router.post("/verify-otp", async (req, res) => {
    ...
    try {
        ...
        const formattedNumber = `+91${contactNumber.replace(/\D/g, "")}`;
        const verificationResponse = await sendOtpService_1.default.verifyOtp(formattedNumber, otp);
...
router.post("/submit-form", async (req, res) => {
    ...
    try {
        ...
        const existingEnquiry = await propertenquiry_1.default.findOne({ email, propertyId });
        if (existingEnquiry) {
        ...
        const newEnquiry = new propertenquiry_1.default({
            name,
            email,
            contactNumber,
            isVerified,
...
router.get("/enquiries", async (_req, res) => {
    try {
        const enquiries = await propertenquiry_1.default.find();
        res.status(200).json({
            success: true,
            data: enquiries,
        });
...
router.get("/enquiryget/:id", async (req, res) => {
    const { id } = req.params;
    try {
        const enquiry = await propertenquiry_1.default.findById(id);
        if (!enquiry) {
            return res.status(404).json({
                success: false,
...
router.put("/enquiryput/:id", async (req, res) => {
    ...
    const { name, email, contactNumber } = req.body;
    try {
        const updatedEnquiry = await propertenquiry_1.default.findByIdAndUpdate(id, { name, email, contactNumber }, { new: true } // Return updated document
        );
        if (!updatedEnquiry) {
            return res.status(404).json({
                success: false,
...
router.delete("/enquirydel/:id", async (req, res) => {
    const { id } = req.params;
    try {
        const deletedEnquiry = await propertenquiry_1.default.findByIdAndDelete(id);
        if (!deletedEnquiry) {
            return res.status(404).json({
                success: false,
                message: "Property enquiry not found.",
